...
def encode_bech32(hrp, witver, witprog):
    """Encodes a native SegWit P2WPKH address (bc1q...)"""
...
# ==========================================
# BASE58 CHECK LAYER (LEGACY COMPATIBILITY)
# ==========================================
...
def pubkey_to_segwit_address(pubkey_bytes: bytes) -> str:
    """Calculates RIPEMD160 hash to produce the SegWit Witness Program."""
    sha256_bp = hashlib.sha256(pubkey_bytes).digest()
    try:
        ripemd160 = hashlib.new('ripemd160', sha256_bp).digest()
    except ValueError:
...
def load_funded_addresses(filename="funded_address.txt") -> set:
    try:
        with open(filename, "r") as f:
            # Enforce strict lowercase parsing for matching error safety
...
if __name__ == "__main__":
    ...
    report_interval = 250
    auto_save_interval = 5000  # Flush tracking state to device storage every 5k iterations
    ...
    try:
        while current_int < end_range:
            ...
            # 3. Native SegWit (Bech32) Generation - Derived using Compressed Key
            addr_segwit = pubkey_to_segwit_address(pub_compressed)
            
            # Memory Evaluation Pipeline
            derived_set = {addr_legacy_uncomp.lower(), addr_legacy_comp.lower(), addr_segwit.lower()}
